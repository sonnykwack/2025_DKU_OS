 ...
 */
void SkipListTest::load_workload() {
  // random 함수 초기화
 ...
 */
void SkipListTest::run_workload() {
  ...
  for (int i = 0; i < thread_num_; i++){
    // TestCase의 skiplist 포인터 저장
    th_args[i].skiplist = skiplist;
 ...
 * SkipList와 std::map을 순회한 결과가 모두 동일한지 확인한다.
 */
void SkipListTest::check_answer() {
  ...
  Node* current = skiplist->get_header()->forward[0];
  
  // 순회하면서 arr에 값 저장
  for (int i = 0; current != nullptr; i++) {
    arr[i] = {current->key, current->value, current->upd_cnt};
    current = current->forward[0];
  }

  // 정답 검증
  auto iter = map_.begin();
  for (int i = 0; i < map_.size(); i++, iter++) {
...
void* thread_job(void *arg){
  ...
  for (int i=0; i < th_arg->workload_num; i++){
    ...
    switch (th_arg->workload[i].req_type) {
      // 삽입
      case INSERT:
        th_arg->skiplist->insert(th_arg->workload[i].key, th_arg->workload[i].value);
        break;
      // 탐색
      case LOOKUP:
        th_arg->skiplist->lookup(th_arg->workload[i].key);
        break;
      // 삭제
      case DELETE:
        th_arg->skiplist->remove(th_arg->workload[i].key);
        break;
      default:
        // 예외처리
        exit(0);
