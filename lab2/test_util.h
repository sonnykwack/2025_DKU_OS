...
struct ThreadArg {
  ...
  int workload_num;
  // SkipList 포인터
  DefaultSkipList* skiplist;
};

/// @brief 부모 테스트 클래스
class SkipListTest : public ::testing::TestWithParam<std::tuple<WL_TYPE, int, int>> {
 protected:
  // 테스트 워크로드
  Req* workload_;
  // 테스트할 SkipList 포인터
 ...
 public:
   ...
   * SkipList와 std::map을 (중위)순회한 결과가 모두 동일한지 확인한다.
   */
  void check_answer();
};

/// @brief Test SkipList with Single Thread
class SkipListSingleThreadTest : public SkipListTest {};

/// @brief Test SkipList with Multi Thread
class SkipListMultiThreadTest : public SkipListTest {};
